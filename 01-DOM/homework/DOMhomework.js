...
span.innerHTML = `${span.innerHTML} <strong>Jefferson</strong>`;
...
function buildToDo(todo, index) {
  ...
  toDoText.id = index; //se le asigna un id a la etiqueta span
  if (todo.complete) toDoText.className = "completeText";
  toDoText.addEventListener("click", completeToDo);
  toDoShell.appendChild(toDoText); //TodoText es hijo de todoShell
  //<div class = "todoShell">
...
function buildToDos(toDos) {
  //Construye un HTML
  // Tu código acá:
  let arr = toDos.map((e, i) => {
    return buildToDo(e, i); //Con esta funcion
  });
...
function addToDo() {
  ...
  //validacion
  if (input !== "") {
    let miTodo = new ToDo(input);
    toDoItems.push(miTodo);
    var inputText = document.querySelector("#toDoInput");
    inputText.value = "";
    displayToDos();
  } else {
    alert("complete el recuadro");
  }
}
